...
// every change. Version 9 = this build.
//
...
input double TrendSlopeMinATR          = 0.00; // require this much EMA slope per bar, in M5 ATRs; 0 = direction only
...
void ManagePosition()
{
   ...
   if(ticket!=g_posTicket)
   {
      ...
      AdoptTicket(ticket);
      if(previous>=0 && carried>g_posMaxFav)
      {
         g_posMaxFav=carried;
         PersistTicketState(ticket);
      }
...
void CreatePanel()
{
   ...
   PanelText("Title","XVISION  |  GOLD SCALPER M1/M5 V9",PANEL_LABEL_X,22,
             PANEL_TITLE,13);
   ...
   PanelPair("Atr","M5 trend / dist / ATR M1",ROW_ATR);
...
void UpdatePanel(const bool force=false)
{
   ...
   PanelValue("Atr",ROW_ATR,StringFormat("%s / %+.2f / %.2f",
              trendTxt,g_trendDistATR,g_atrM1),
