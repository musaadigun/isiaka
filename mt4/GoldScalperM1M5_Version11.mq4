...
// v6.00: MaxShockExhaustion now uses 0 = off, matching every other
...
//        gate instead of only the first, and LogGateDiagnostics writes
...
input double CusumTrigger              = 2.00;  // evidence needed to call a burst [3.00]
input double CusumAllowance            = 0.18;  // per-bar noise deadband
input double CusumDecay                = 0.94;  // how fast old evidence fades; higher = more patient
input int    CusumFreshBars            = 5;     // enter within N bars of the crossing; 0 = off [3]
input double MinEfficiencyRatio        = 0.10;  // path directness 0..1; 0 = off [0.30]
input double MinM1Strength             = 0.05;  // M1 velocity in ATRs; 0 = off [0.10]
input double MinM1Coherence            = 0.50;  // fraction of M1 windows agreeing; 0 = off [0.75]
input bool   RequireBarBodyAligned     = true;  // trigger bar must close in the signal direction
input double MaxBarSizeATR             = 0.00;  // reject bars over N x ATR; 0 = off [2.00]
input double MaxMoveMaturityATR        = 0.00;  // reject moves N x M5 ATR old; 0 = off [1.50]
input bool   RequireM5Alignment        = false; // M5 velocity and EMA ribbon must agree [true]
input double MaxShockExhaustion        = 0.00;  // stand down above this shock+exhaustion; 0 = off [0.45]
input double MinImpulseDrift           = 0.00;  // require this much impulse+drift; 0 = off [0.35]
input int    CooldownSeconds           = 120;   // minimum seconds between entries; 0 = off
input bool   LogGateDiagnostics        = false; // write one ledger row per blocked bar
...
input int    TrendFilterMode           = 0;    // 0 = off, 1 = block against trend, 2 = also require a pullback
input int    M5TrendEMAPeriod          = 50;   // EMA period on M5
input double TrendSlopeMinATR          = 0.00; // minimum EMA slope per bar in M5 ATRs. 0 = side-of-line only,
                                               // so a flat EMA still gives a side. Raise it to make chop report none.
input double TrendMaxDistanceATR       = 1.50; // mode 2: entry must be within this many M5 ATRs of the line
...
// Regime router (ER_PERIOD is structural: it sizes the lookback)
...
const bool   WRITE_DATASET         = true;   // per-trade CSV with MFE/MAE + entry conditions
...
double   g_lastCoherence=0.0, g_lastMaturity=0.0;   // recorded every bar for the dataset
double   g_trendEma=0.0;        // M5 slow EMA, the directional anchor
int      g_trendDir=0;          // +1 above and rising, -1 below and falling, 0 undecided
double   g_trendDistATR=0.0;    // how far price sits from the line, in M5 ATRs
double   g_feat[GS_FEAT_COUNT];
string   g_featKey[GS_FEAT_COUNT]={"MF","MA","ER","CU","CD","MS","CH","MT",
...
datetime g_dcOpenOf[200];      // open time of each closed row, for grouping
...
void UpdateCusum(const datetime barTime)
{
   ...
   if(prevUp<CusumTrigger && g_cusumUp>=CusumTrigger)     g_cusumUpCross=barTime;
   if(prevDown<CusumTrigger && g_cusumDown>=CusumTrigger) g_cusumDownCross=barTime;
}
...
void UpdateTrend()
{
   ...
   g_trendDir=0;
   g_trendDistATR=0.0;
   if(M5TrendEMAPeriod<2) return;

   double ema=iMA(Symbol(),PERIOD_M5,M5TrendEMAPeriod,0,MODE_EMA,PRICE_CLOSE,1);
   double emaPrev=iMA(Symbol(),PERIOD_M5,M5TrendEMAPeriod,0,MODE_EMA,PRICE_CLOSE,2);
   if(ema<=0.0 || emaPrev<=0.0) return;
   g_trendEma=ema;

   double close5=iClose(Symbol(),PERIOD_M5,1);
   if(g_atrM5>0.0) g_trendDistATR=(close5-ema)/g_atrM5;

   double slopeATR=(g_atrM5>0.0 ? (ema-emaPrev)/g_atrM5 : 0.0);
   bool aboveRising=(close5>ema && slopeATR>=TrendSlopeMinATR);
   bool belowFalling=(close5<ema && -slopeATR>=TrendSlopeMinATR);
   if(aboveRising)       g_trendDir=1;
   else if(belowFalling) g_trendDir=-1;
}
...
void AddGateFailure(string &list,const string token)
{
...
int EvaluateMomentum(string &blocker)
{
   ...
   datetime cross=0;
   bool burst=false;
   if(g_cusumUp>=CusumTrigger && g_cusumUp-g_cusumDown>=CusumTrigger*0.5)
      { dir=1; cross=g_cusumUpCross; burst=true; }
   else if(g_cusumDown>=CusumTrigger && g_cusumDown-g_cusumUp>=CusumTrigger*0.5)
      { dir=-1; cross=g_cusumDownCross; burst=true; }
   if(dir==0) dir=(g_m1Comp>=0.0 ? 1 : -1);
   ...
   g_lastCoherence=M1Coherence(1,dir);
   g_lastMaturity=0.0;
   if(g_atrM5>0.0)
   {
      ...
      g_lastMaturity=(dir>0 ? mclose-mlo : mhi-mclose)/g_atrM5;
   }

   string fails="";

   if(!burst)
      AddGateFailure(fails,StringFormat("burst %.1f/%.1f",
                     MathMax(g_cusumUp,g_cusumDown),CusumTrigger));
   else if(CusumFreshBars>0 &&
           (cross==0 || (iTime(Symbol(),PERIOD_M1,1)-cross)>CusumFreshBars*60))
      AddGateFailure(fails,"stale");

   if(MaxShockExhaustion>0.0 && g_modeShock+g_modeExhaustion>MaxShockExhaustion)
      AddGateFailure(fails,StringFormat("shock/exh %.2f",g_modeShock+g_modeExhaustion));

   if(MinImpulseDrift>0.0 && g_modeImpulse+g_modeDrift<MinImpulseDrift)
      AddGateFailure(fails,StringFormat("imp+drift %.2f",g_modeImpulse+g_modeDrift));

   if(MinEfficiencyRatio>0.0 && (dir*g_er<=0.0 || MathAbs(g_er)<MinEfficiencyRatio))
      AddGateFailure(fails,StringFormat("ER %+.2f",g_er));

   if(MinM1Strength>0.0 && dir*g_m1Comp<MinM1Strength)
      AddGateFailure(fails,StringFormat("M1str %.2f",dir*g_m1Comp));

   if(MinM1Coherence>0.0 && g_lastCoherence<MinM1Coherence)
      AddGateFailure(fails,StringFormat("coher %.2f",g_lastCoherence));

   if(RequireBarBodyAligned)
   {
      double body=iClose(Symbol(),PERIOD_M1,1)-iOpen(Symbol(),PERIOD_M1,1);
      if(dir*body<=0.0) AddGateFailure(fails,"body");
   }

   if(MaxBarSizeATR>0.0 && g_atrM1>0.0)
   {
      double tr=iHigh(Symbol(),PERIOD_M1,1)-iLow(Symbol(),PERIOD_M1,1);
      if(tr>MaxBarSizeATR*g_atrM1)
         AddGateFailure(fails,StringFormat("bigbar %.1fx",tr/g_atrM1));
   }

   if(MaxMoveMaturityATR>0.0 && g_atrM5>0.0 && g_lastMaturity>MaxMoveMaturityATR)
      AddGateFailure(fails,StringFormat("mature %.1fx",g_lastMaturity));

   if(TrendFilterMode>0)
   {
      if(g_trendDir==0)
         AddGateFailure(fails,"no M5 trend");
      else if(g_trendDir!=dir)
         AddGateFailure(fails,StringFormat("vs M5 trend (%s)",g_trendDir>0?"up":"down"));
      else if(TrendFilterMode>=2 && TrendMaxDistanceATR>0.0 &&
              MathAbs(g_trendDistATR)>TrendMaxDistanceATR)
         AddGateFailure(fails,StringFormat("far from EMA %.1fx",MathAbs(g_trendDistATR)));
   }

   if(RequireM5Alignment)
   {
      if(dir*g_m5Comp<=0.0) AddGateFailure(fails,"M5vel");
      double e20=iMA(Symbol(),PERIOD_M5,20,0,MODE_EMA,PRICE_CLOSE,1);
      double e30=iMA(Symbol(),PERIOD_M5,30,0,MODE_EMA,PRICE_CLOSE,1);
      if(g_atrM5>0.0 && dir*(e20-e30)/g_atrM5<-0.05) AddGateFailure(fails,"M5ribbon");
   }
...
void RefreshDayCache()
{
   ...
   for(int g=0;g<n;g++)
   {
      if(g_dcOpenOf[g]==0) continue;                 // already merged away
      for(int h=g+1;h<n;h++)
      {
         if(g_dcOpenOf[h]!=g_dcOpenOf[g]) continue;
         g_dcProfits[g]+=g_dcProfits[h];
         if(g_dcCloseTimes[h]>g_dcCloseTimes[g])
            g_dcCloseTimes[g]=g_dcCloseTimes[h];     // trade ends with its last leg
         g_dcOpenOf[h]=0;
         g_dcProfits[h]=0.0;
         g_dcCloseTimes[h]=0;
      }
   ...
   for(int c=0;c<n;c++)
   {
      if(g_dcOpenOf[c]==0 && g_dcCloseTimes[c]==0) continue;
      g_dcCloseTimes[m]=g_dcCloseTimes[c];
      g_dcProfits[m]=g_dcProfits[c];
      m++;
   }
...
void DatasetWrite(const double profit,const string reason)
{
   if(!WRITE_DATASET) return;
   ...
   FileWrite(h,
      ...
      DoubleToString(g_feat[F_MFE],2),
      DoubleToString(g_feat[F_MAE],2),
      DoubleToString(g_feat[F_ER],3),
      DoubleToString(g_feat[F_CUSUP],2),
      DoubleToString(g_feat[F_CUSDN],2),
      DoubleToString(g_feat[F_M1STR],3),
      DoubleToString(g_feat[F_COHER],2),
      DoubleToString(g_feat[F_MATUR],2),
      DoubleToString(g_feat[F_ATRM1],3),
      DoubleToString(g_feat[F_ATRM5],3),
      DoubleToString(g_feat[F_EXPAN],2),
      DoubleToString(g_feat[F_SPRED],3),
      DoubleToString(g_feat[F_NOISE],2),
      DoubleToString(g_feat[F_DRIFT],2),
      DoubleToString(g_feat[F_IMPUL],2),
      DoubleToString(g_feat[F_EXHST],2),
      DoubleToString(g_feat[F_SHOCK],2),
      DoubleToString(g_feat[F_TRDIR],0),
      DoubleToString(g_feat[F_TRDST],2));
   FileClose(h);
}

void JournalClosedTicket(const int ticket)
{
   if(ticket<0) return;
   if(!OrderSelect(ticket,SELECT_BY_TICKET,MODE_HISTORY)) return;
   if(OrderSymbol()!=Symbol() || OrderMagicNumber()!=MAGIC_NUMBER) return;
   if(OrderCloseTime()<=0) return;
   if(OrderType()!=OP_BUY && OrderType()!=OP_SELL) return;
   double profit=OrderProfit()+OrderSwap()+OrderCommission();
   string reason=InferredCloseReason();
   // MFE/MAE live in g_feat only after PersistTicketState has synced
   // them; do it here so a close that happens between syncs is exact.
   g_feat[F_MFE]=g_posMaxFav;
   g_feat[F_MAE]=g_posMaxAdv;
   LedgerWrite("EXIT",(OrderType()==OP_BUY?1:-1),OrderLots(),OrderClosePrice(),
               profit,reason);
   DatasetWrite(profit,reason);
   g_lastAction=StringFormat("CLOSED %+.2f (peak %+.2f dip %.2f): %s",
                             profit,g_posMaxFav,g_posMaxAdv,reason);
   if(PUSH_ALERTS)
...
string AccountKey(const string suffix)
{
   return(StringFormat("GS1.%d.%d.%s",AccountNumber(),MAGIC_NUMBER,suffix));
}
...
void SnapshotEntryFeatures(const double spreadAtEntry)
{
   g_feat[F_MFE]=0.0;
   g_feat[F_MAE]=0.0;
   g_feat[F_ER]=g_er;
   g_feat[F_CUSUP]=g_cusumUp;
   g_feat[F_CUSDN]=g_cusumDown;
   g_feat[F_M1STR]=g_m1Comp;
   g_feat[F_COHER]=g_lastCoherence;
   g_feat[F_MATUR]=g_lastMaturity;
   g_feat[F_ATRM1]=g_atrM1;
   g_feat[F_ATRM5]=g_atrM5;
   g_feat[F_EXPAN]=g_expansion;
   g_feat[F_SPRED]=spreadAtEntry;
   g_feat[F_NOISE]=g_modeNoise;
   g_feat[F_DRIFT]=g_modeDrift;
   g_feat[F_IMPUL]=g_modeImpulse;
   g_feat[F_EXHST]=g_modeExhaustion;
   g_feat[F_SHOCK]=g_modeShock;
   g_feat[F_TRDIR]=(double)g_trendDir;
   g_feat[F_TRDST]=g_trendDistATR;
}
...
void PersistTicketState(const int ticket)
{
   ...
   g_feat[F_MFE]=g_posMaxFav;
   g_feat[F_MAE]=g_posMaxAdv;
   for(int i=0;i<GS_FEAT_COUNT;i++)
      GlobalVariableSet(TicketKey(ticket,g_featKey[i]),g_feat[i]);
}
...
void PersistExcursions(const int ticket)
{
   if(IsTesting() || ticket<0) return;
   g_feat[F_MFE]=g_posMaxFav;
   g_feat[F_MAE]=g_posMaxAdv;
   GlobalVariableSet(TicketKey(ticket,g_featKey[F_MFE]),g_feat[F_MFE]);
   GlobalVariableSet(TicketKey(ticket,g_featKey[F_MAE]),g_feat[F_MAE]);
}
...
void DropTicketState(const int ticket)
{
   if(IsTesting() || ticket<0) return;
   for(int i=0;i<GS_FEAT_COUNT;i++)
      GlobalVariableDel(TicketKey(ticket,g_featKey[i]));
}
...
void AdoptTicket(const int ticket)
{
   ...
   g_posMaxFav=MathMax(0.0,move);
   g_posMaxAdv=MathMin(0.0,move);
   if(!IsTesting())
   {
      ...
      for(int i=0;i<GS_FEAT_COUNT;i++)
      {
         string key=TicketKey(ticket,g_featKey[i]);
         if(GlobalVariableCheck(key)) g_feat[i]=GlobalVariableGet(key);
      }
      g_posMaxFav=MathMax(g_posMaxFav,g_feat[F_MFE]);
      g_posMaxAdv=MathMin(g_posMaxAdv,g_feat[F_MAE]);
...
bool CloseSelectedOrder(const string reason)
{
   ...
   for(int attempt=1;attempt<=GS_CLOSE_RETRY_ATTEMPTS;attempt++)
   {
      ...
      if(OrderClose(ticket,lots,price,SlippagePoints(),clrSilver))
      {
         ...
         // trades go unrecorded. ForgetPosition() below zeroes the
...
bool SendEntry(const int dir,const int module,const double fadeTarget)
{
   RefreshRates();
   ...
   double slDist=StopLoss_PriceUSD;
   double tpDist=TakeProfit_PriceUSD;
   if(slDist>0.0 && minDist>0.0 && slDist<minDist)
   {
      Print("GoldScalper: StopLoss_PriceUSD ",DoubleToString(StopLoss_PriceUSD,2),
   ...
   {
      Print("GoldScalper: TakeProfit_PriceUSD ",DoubleToString(TakeProfit_PriceUSD,2),
            " is inside the broker minimum ",DoubleToString(minDist,2),
   ...
   if(!IsTesting()) GlobalVariableSet(AccountKey("LASTENTRY"),(double)g_lastEntryTime);
   InvalidateDayCache();
...
bool EnsureStopProtection()
{
   ...
   double minDist=BrokerModifyDistance();
   double repairDist=MathMax(StopLoss_PriceUSD,minDist);
...
void ManagePosition()
{
   ...
   if(ticket<0)
   {
      if(g_posTicket>=0)
      {
         JournalClosedTicket(g_posTicket);
         ForgetPosition();
         InvalidateDayCache();
      }
      return;
   ...
   if(ticket!=g_posTicket)
   {
      ...
      int previous=g_posTicket;
      double carriedFav=g_posMaxFav;
      double carriedAdv=g_posMaxAdv;
      if(previous>=0)
      {
         JournalClosedTicket(previous);          // selects the history order
         DropTicketState(previous);
      ...
      if(previous>=0)
      {
         ...
         if(carriedFav>g_posMaxFav) { g_posMaxFav=carriedFav; carriedSomething=true; }
         if(carriedAdv<g_posMaxAdv) { g_posMaxAdv=carriedAdv; carriedSomething=true; }
         if(carriedSomething) PersistTicketState(ticket);
      }
   }
   ...
   if(movement>g_posMaxFav)
   {
      ...
      if(persist) PersistExcursions(ticket);
   }
   if(movement<g_posMaxAdv)
   {
      bool persistAdv=(g_posMaxAdv-movement>0.05);
      g_posMaxAdv=movement;
      if(persistAdv) PersistExcursions(ticket);
   }
...
void CreatePanel()
{
   ...
   PanelText("Title","XVISION  |  GOLD SCALPER M1/M5 V11",PANEL_LABEL_X,22,
             PANEL_TITLE,13);
   ...
   PanelPair("Blk","Gates failing",ROW_BLK);
   ...
   PanelPair("Cus","ER / CUSUM up / dn",ROW_CUS);
   PanelPair("Atr","Trend / ATR M1 / M5 / exp",ROW_ATR);
...
void UpdatePanel(const bool force=false)
{
   ...
   PanelValue("Blk",ROW_BLK,StringLen(g_blocker)==0 ? "clear" : PanelFit(g_blocker,46),
              StringLen(g_blocker)==0 ? PANEL_GREEN : PANEL_AMBER);

   if(havePos)
   {
      double move=ProfitMovementSelected();
      PanelValue("Pos",ROW_POS,StringFormat("%s %.2f  %+.2f   peak %+.2f  dip %.2f",
                 OrderType()==OP_BUY?"BUY":"SELL",OrderLots(),move,
                 g_posMaxFav,g_posMaxAdv),
   ...
   PanelValue("Own",ROW_OWN,ownership,
              StopLoss_PriceUSD<=0.0 ? PANEL_AMBER : PANEL_VALUE);

   //--- engine
   PanelValue("Reg",ROW_REG,StringFormat("%.2f  %.2f  %.2f  %.2f  %.2f",
              g_modeNoise,g_modeDrift,g_modeImpulse,g_modeExhaustion,g_modeShock),
              (MaxShockExhaustion>0.0 &&
               g_modeShock+g_modeExhaustion>MaxShockExhaustion)?PANEL_AMBER:PANEL_VALUE);
   PanelValue("Cus",ROW_CUS,StringFormat("%+.2f  /  %.1f / %.1f  (need %.1f)",
              g_er,g_cusumUp,g_cusumDown,CusumTrigger),
              (MathMax(g_cusumUp,g_cusumDown)>=CusumTrigger)?PANEL_GREEN:PANEL_VALUE);
   string trendTxt=(TrendFilterMode<=0 ? "off" :
                    (g_trendDir>0 ? "UP" : (g_trendDir<0 ? "DOWN" : "flat")));
   PanelValue("Atr",ROW_ATR,StringFormat("%s %+.2f / %.2f / %.2f / %.2fx",
              trendTxt,g_trendDistATR,g_atrM1,g_atrM5,g_expansion),
              (TrendFilterMode>0 && g_trendDir>0) ? PANEL_GREEN :
              ((TrendFilterMode>0 && g_trendDir<0) ? PANEL_MAGENTA : PANEL_VALUE));
...
bool ValidateInputs()
{
   ...
   if(CusumTrigger<=0.0 || CusumAllowance<0.0 || CusumDecay<=0.0 || CusumDecay>=1.0)
   {
      Print("GoldScalper: CusumTrigger must be positive, CusumAllowance non-negative, ",
            "and CusumDecay strictly between 0 and 1.");
      return(false);
   }
   if(MinM1Coherence<0.0 || MinM1Coherence>1.0 ||
      MinEfficiencyRatio<0.0 || MinEfficiencyRatio>1.0 ||
      MaxShockExhaustion<0.0 || MaxShockExhaustion>1.0 ||
      MinImpulseDrift<0.0 || MinImpulseDrift>1.0)
   {
      Print("GoldScalper: MinM1Coherence, MinEfficiencyRatio, MaxShockExhaustion ",
            "and MinImpulseDrift are fractions and must be between 0 and 1.");
      return(false);
   }
   if(TrendFilterMode<0 || TrendFilterMode>2)
   {
      Print("GoldScalper: TrendFilterMode must be 0 (off), 1 (block against) or 2 (pullback only).");
      return(false);
   }
   if(TrendFilterMode>0 && M5TrendEMAPeriod<2)
   {
      Print("GoldScalper: M5TrendEMAPeriod must be 2 or more when the trend filter is on.");
      return(false);
   }
   if(TrendSlopeMinATR<0.0 || TrendMaxDistanceATR<0.0)
   {
      Print("GoldScalper: trend slope and distance limits cannot be negative.");
      return(false);
   }
   if(MinM1Strength<0.0 || MaxBarSizeATR<0.0 || MaxMoveMaturityATR<0.0 ||
      CusumFreshBars<0 || CooldownSeconds<0)
   {
...
void CleanOrphanedTicketState()
{
   ...
   for(int i=GlobalVariablesTotal()-1;i>=0;i--)
   {
      ...
      // Account-level keys (AccountKey, e.g. "...LASTENTRY") carry no
...
int OnInit()
{
   ...
   Print("GoldScalper: entry gates active - burst>=",DoubleToString(CusumTrigger,2),
         (CusumFreshBars>0?StringFormat(" within %d bars",CusumFreshBars):""),
         (MinEfficiencyRatio>0.0?", ER>="+DoubleToString(MinEfficiencyRatio,2):""),
         (MinM1Strength>0.0?", M1str>="+DoubleToString(MinM1Strength,2):""),
         (MinM1Coherence>0.0?", coher>="+DoubleToString(MinM1Coherence,2):""),
         (RequireBarBodyAligned?", body":""),
         (MaxBarSizeATR>0.0?", bar<="+DoubleToString(MaxBarSizeATR,2)+"xATR":""),
         (MaxMoveMaturityATR>0.0?", maturity<="+DoubleToString(MaxMoveMaturityATR,2):""),
         (RequireM5Alignment?", M5 aligned":""),
         (MaxShockExhaustion>0.0?", shock+exh<="+DoubleToString(MaxShockExhaustion,2):""),
         (MinImpulseDrift>0.0?", imp+drift>="+DoubleToString(MinImpulseDrift,2):""),
         (TrendFilterMode>0?StringFormat(", M5 EMA%d trend filter mode %d",
                                         M5TrendEMAPeriod,TrendFilterMode):""),
         ".");
   ...
   if(minDist>0.0)
   {
      if(StopLoss_PriceUSD>0.0 && StopLoss_PriceUSD<minDist)
         Print("GoldScalper: WARNING - StopLoss_PriceUSD ",
               DoubleToString(StopLoss_PriceUSD,2),
               " is inside that minimum; stops will be placed at ",
               DoubleToString(minDist,2)," instead.");
      if(TakeProfit_PriceUSD>0.0 && TakeProfit_PriceUSD<minDist)
         Print("GoldScalper: WARNING - TakeProfit_PriceUSD ",
               DoubleToString(TakeProfit_PriceUSD,2),
               " is inside that minimum; targets will be placed at ",
               DoubleToString(minDist,2)," instead.");
   }

   if(!IsTesting() && GlobalVariableCheck(AccountKey("LASTENTRY")))
      g_lastEntryTime=(datetime)GlobalVariableGet(AccountKey("LASTENTRY"));
...
void OnTick()
{
   ...
   if(bar>0 && bar!=g_lastM1Bar)
   {
      ...
      int neededM5=40;
      if(TrendFilterMode>0) neededM5=MathMax(neededM5,M5TrendEMAPeriod+5);
      int haveM1=iBars(Symbol(),PERIOD_M1);
      int haveM5=iBars(Symbol(),PERIOD_M5);
      if(haveM1>ER_PERIOD+60 && haveM5>neededM5)
      {
         ...
         if(dir!=0)
         {
            ...
            if(EntryAllowed(dir,railBlocker))
            {
               ...
               if(SendEntry(dir,module,fadeTarget))
                  g_blocker="";
               else
               {
                  g_blocker=g_lastAction;
                  if(LogGateDiagnostics)
                     LedgerWrite("BLOCKED",dir,0.0,g_signalRef,0.0,"send: "+g_lastAction);
               }
            }
            else
            {
               ...
               if(LogGateDiagnostics)
                  LedgerWrite("BLOCKED",dir,0.0,g_signalRef,0.0,"rail: "+railBlocker);
            }
         }
         else
         {
            // every gate that failed this bar, not just the first
            if(UseMomentumModule && USE_FADE)
            ...
            if(LogGateDiagnostics)
               LedgerWrite("BLOCKED",0,0.0,g_signalRef,0.0,
                           StringLen(g_blocker)>0 ? g_blocker : "no failure recorded");
         }
      }
      else
         g_blocker=StringFormat("history M1 %d/%d, M5 %d/%d",
                                haveM1,ER_PERIOD+61,haveM5,neededM5+1);
   }
   UpdatePanel();
}
