...
// every change. Version 6 = this build.
//
...
int EvaluateMomentum(string &blocker)
{
   ...
   // can still be reported against a plausible direction.
   int dir=0;
   datetime cross=0;
   bool burst=false;
   ...
   if(MinM1Coherence>0.0 && M1Coherence(1,dir)<MinM1Coherence)
      AddGateFailure(fails,StringFormat("coher %.2f",M1Coherence(1,dir)));
...
void CreatePanel()
{
   ...
   PanelText("Title","XVISION  |  GOLD SCALPER M1/M5 V6",PANEL_LABEL_X,22,
             PANEL_TITLE,13);
      ...
      if(iBars(Symbol(),PERIOD_M1)>ER_PERIOD+60 && iBars(Symbol(),PERIOD_M5)>40)
      {
         ...
         if(dir!=0)
         {
            ...
            if(EntryAllowed(dir,railBlocker))
            {
               ...
               g_blocker=(SendEntry(dir,module,fadeTarget) ? "" : g_lastAction);
            }
            else
            {
