...
// every change. Version 7 = this build.
//
...
int EvaluateMomentum(string &blocker)
{
   ...
   if(MinM1Coherence>0.0)
   {
      double coher=M1Coherence(1,dir);
      if(coher<MinM1Coherence) AddGateFailure(fails,StringFormat("coher %.2f",coher));
   }
   ...
   if(MaxMoveMaturityATR>0.0 && g_atrM5>0.0)
   {
      ...
      double maturity=(dir>0 ? close1-lo : hi-close1)/g_atrM5;
      if(maturity>MaxMoveMaturityATR)
         AddGateFailure(fails,StringFormat("mature %.1fx",maturity));
   }
...
void JournalClosedTicket(const int ticket)
{
   ...
   LedgerWrite("EXIT",(OrderType()==OP_BUY?1:-1),OrderLots(),OrderClosePrice(),
               profit,reason);
   g_lastAction=StringFormat("CLOSED %+.2f: %s",profit,reason);
   if(PUSH_ALERTS)
...
void CreatePanel()
{
   ...
   PanelText("Title","XVISION  |  GOLD SCALPER M1/M5 V7",PANEL_LABEL_X,22,
             PANEL_TITLE,13);
