#!/usr/bin/env python3
"""Tick-replay backtester for GoldScalperM1M5 (mirrors EA Version 11).
...
class Config:
    ...
    # Exit engine. Mirrors EA v11: the ONLY exits are the user's SL, TP,
    ...
    # momentum module - mirrors EA v11 defaults (permissive; 0 = gate off)
...
class Backtester:
    ...
    def update_regime(self):
        ...
        # Mirrors the EA: no trend reading until the EMA has enough M5
        # history, otherwise the seed point moves between shifts and the
        # slope is noise.
        self.trend_dir, self.trend_dist_atr = 0, 0.0
        if (cfg.m5_trend_ema_period >= 2
                and self.m5.count() >= cfg.m5_trend_ema_period * 4 + 4):
