...
//|     and acceleration. The study measured 3-bar price velocity and|
//|     its one-bar change; both calculations are fixed and explicit.|
...
input double             MinimumDirectionalAcceleration     = 0.00;
...
// bar gradient / velocity / acceleration / range-vote filters qualify,
...
double   g_lastDirectionalAcceleration=0.0;
...
double   g_minDirectionalAcceleration=0.00;
...
void ResolveInputs()
  {
   ...
   ClampDouble("MinimumDirectionalAcceleration",MinimumDirectionalAcceleration,
               0.0,1000000.0,g_minDirectionalAcceleration);
...
void ProcessNewSignalBar()
  {
   ...
   g_lastDirectionalAcceleration=direction*g_currentAcceleration;
   ...
   bool researchSignal=(g_lastDirectionalGradient>=g_minDirectionalGradient &&
                        ...
                        g_lastDirectionalAcceleration>g_minDirectionalAcceleration &&
   ...
   if(PrintSignalDiagnostics)
     {
       Print("XVISION EMA20 V10 cross ",DirectionName(direction),
            ...
            " acceleration=",DoubleToString(g_lastDirectionalAcceleration,4),
...
//| CLOSED bars - gradient, velocity, acceleration and range votes   |
...
void ProcessInstantCrossEntry()
  {
   ...
   double acceleration=direction*g_currentAcceleration;
   if(gradient<g_minDirectionalGradient ||
      velocity<=g_minDirectionalVelocity ||
      acceleration<=g_minDirectionalAcceleration ||
   ...
   if(PrintSignalDiagnostics)
      Print("XVISION EMA20 V10 instant cross ",DirectionName(direction),
            ...
            " acceleration=",DoubleToString(acceleration,4),
...
void ProcessPendingMarketEntry()
  {
   ...
   if(direction*g_currentAcceleration<=g_minDirectionalAcceleration)
     {
      g_lastDecision="Market entry skipped: directional acceleration no longer qualifies";
      return;
     }
   ...
   if(PrintSignalDiagnostics)
      Print("XVISION EMA20 V10 confirmation ",DirectionName(direction),
            ...
            " acceleration=",DoubleToString(direction*g_currentAcceleration,4),
            " rangeVotes=",g_currentRangeVotes);
...
//+------------------------------------------------------------------+
//| Change in rolling velocity over the latest completed candle.     |
//+------------------------------------------------------------------+
double PriceAccelerationPerHour2(const int shift)
  {
   int timeframeSeconds=PeriodSeconds(SignalTimeframe);
   if(timeframeSeconds<=0)
      return(0.0);
   double barHours=timeframeSeconds/3600.0;
   if(barHours<=0.0)
      return(0.0);
   double velocityNow=PriceVelocityPerHour(shift);
   double velocityPrevious=PriceVelocityPerHour(shift+1);
   return((velocityNow-velocityPrevious)/barHours);
  }
...
void UpdateCurrentDiagnostics()
  {
   ...
   g_currentAcceleration=PriceAccelerationPerHour2(1);
...
bool RetestCandleQualifies(const int direction,string &reason)
  {
   ...
   if(direction*g_currentAcceleration<=g_minDirectionalAcceleration)
     { reason="directional acceleration is too weak"; return(false); }
   if(g_currentRangeVotes>g_maxRangeVotes)
   ...
   if(g_retestState==RETEST_WAIT_MOVE)
     {
      if(HasTrendCloses(g_retestDirection) &&
         ...
         g_retestDirection*g_currentAcceleration>g_minDirectionalAcceleration &&
         g_currentRangeVotes<=g_maxRangeVotes &&
...
void UpdateDashboard(const bool force=false)
  {
   ...
   SetDashboardLabel("MOTION","Velocity  "+DoubleToString(g_currentVelocity,2)+
                     "/h  |  acceleration  "+DoubleToString(g_currentAcceleration,2)+"/h2",
