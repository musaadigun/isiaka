//+------------------------------------------------------------------+
//| supergold.mq4                                                    |
//| Three complete XVISION engines behind one ActiveEngine switch.   |
//|                                                                  |
//|   XVISION_GOLDSCALPER_V11   GoldScalper M1/M5 v11                |
//|   XVISION_SUPERSCALPER_V5   SuperScalper v5                      |
//|   XVISION_EMA20_DIRECTIONAL EMA20 crossing / retest engine       |
//|                                                                  |
//| Exactly one engine initialises and trades per chart. The others  |
//| are compiled in but never touched: OnInit/OnDeinit/OnTick/       |
//| OnTimer all dispatch on ActiveEngine, so a dormant engine never  |
//| reads a quote, places an order, writes a global variable or      |
//| draws an object.                                                 |
//|                                                                  |
//| MERGE NOTES                                                      |
//| The third engine is XVISION_Gold_EMA20_Directional_EA v12, NOT   |
//| the v10 that was supplied. v10 carried three critical defects    |
//| found in audit: a retest engine that could never reach ARMED, an |
//| instant-cross path that blacked itself out for four bars after   |
//| any crossing it missed, and a retry interval that could be       |
//| clamped to zero and resend a failing order on every tick. v12 is |
//| v10 plus those fixes plus the removal of the acceleration        |
//| filter, which measured 50.2% on +$5 before -$5 against a 44.0-   |
//| 53.1% random band and whose sign flipped on 50.2% of bars.       |
//|                                                                  |
//| Every identifier of the EMA20 engine except its 60 inputs now    |
//| carries an EMA20_ prefix, matching the GSV11_ and SSV5_          |
//| convention already used here. Input NAMES are unchanged so       |
//| existing .set files still load. 148 identifiers were prefixed    |
//| and no string literal was altered by the rename.                 |
//|                                                                  |
//| Verified before merge: zero collisions on inputs, file-scope     |
//| globals, #defines, enums or structs between the three engines.   |
//| The only clashes were OnInit/OnDeinit/OnTick, now routed. Magic  |
//| numbers are distinct: 26082601, 26081410 and 50503006, so two    |
//| engines cannot adopt each other's positions. Panel object        |
//| prefixes GSA_PANEL_ and XVE9_PANEL_ are distinct.                |
//|                                                                  |
//| The EMA20 engine has no OnTimer handler of its own; its branch   |
//| of the timer router is deliberately empty.                       |
//+------------------------------------------------------------------+
#property strict
...
#define EMA20_RESEARCH_VELOCITY_LOOKBACK_BARS      3
...
double   EMA20_g_minDirectionalVelocity=0.00;
...
void EMA20_ResolveInputs()
  {
   ...
   EMA20_ClampDouble("MinimumDirectionalVelocity",MinimumDirectionalVelocity,
               0.0,1000000.0,EMA20_g_minDirectionalVelocity);
...
void EMA20_ProcessNewSignalBar()
  {
   ...
   int motionDeepestShift=2+EMA20_RESEARCH_VELOCITY_LOOKBACK_BARS;
   int maximumShift=(int)MathMax(2+EMA20_RESEARCH_GRADIENT_LOOKBACK_BARS,
                                 MathMax(motionDeepestShift,EMA20_g_rangeLookback+1));
   ...
   EMA20_g_lastDirectionalVelocity=direction*EMA20_g_currentVelocity;
   ...
   bool researchSignal=(EMA20_g_lastDirectionalGradient>=EMA20_g_minDirectionalGradient &&
                        EMA20_g_lastDirectionalVelocity>EMA20_g_minDirectionalVelocity &&
   ...
   if(PrintSignalDiagnostics)
     {
       Print("XVISION EMA20 V12 cross ",EMA20_DirectionName(direction),
            ...
            " velocity=",DoubleToString(EMA20_g_lastDirectionalVelocity,4),
   ...
   if(route=="")
     {
      EMA20_g_lastDecision="Cross rejected by EMA20 gradient / motion / range rules";
      EMA20_SaveEpisodeState();
      return;
     }
...
void EMA20_ProcessInstantCrossEntry()
  {
   ...
   double velocity=direction*EMA20_g_currentVelocity;
   if(gradient<EMA20_g_minDirectionalGradient ||
      velocity<=EMA20_g_minDirectionalVelocity ||
      EMA20_g_currentRangeVotes>EMA20_g_maxRangeVotes)
     {
      EMA20_g_lastDecision="Instant cross rejected by gradient / motion / range rules";
      return;
     }
...
void EMA20_ProcessPendingMarketEntry()
  {
   ...
   if(direction*EMA20_g_currentVelocity<=EMA20_g_minDirectionalVelocity)
     {
      EMA20_g_lastDecision="Market entry skipped: directional velocity no longer qualifies";
      return;
     }
   ...
   if(PrintSignalDiagnostics)
      Print("XVISION EMA20 V12 confirmation ",EMA20_DirectionName(direction),
            ...
            " velocity=",DoubleToString(direction*EMA20_g_currentVelocity,4),
            " rangeVotes=",EMA20_g_currentRangeVotes);
...
void EMA20_UpdateCurrentDiagnostics()
  {
   ...
   EMA20_g_currentVelocity=EMA20_PriceVelocityPerHour(1);
   ...
   if(EMA20_g_retestState==EMA20_RETEST_WAIT_MOVE)
     {
      if(EMA20_HasTrendCloses(EMA20_g_retestDirection) &&
         ...
         EMA20_g_retestDirection*EMA20_g_currentVelocity>EMA20_g_minDirectionalVelocity &&
         EMA20_g_currentRangeVotes<=EMA20_g_maxRangeVotes &&
...
void EMA20_UpdateDashboard(const bool force=false)
  {
   ...
   EMA20_SetDashboardLabel("MOTION","Velocity  "+DoubleToString(EMA20_g_currentVelocity,2)+
                     " $/h  (min "+DoubleToString(EMA20_g_minDirectionalVelocity,2)+")",
...
void OnDeinit(const int reason)
  {
   ...
   else if(ActiveEngine==XVISION_EMA20_DIRECTIONAL) EMA20_OnDeinit(reason);
   else                                             GSV11_OnDeinit(reason);
  }
void OnTick()
  {
   if(ActiveEngine==XVISION_SUPERSCALPER_V5)        SSV5_OnTick();
   else if(ActiveEngine==XVISION_EMA20_DIRECTIONAL) EMA20_OnTick();
   else                                             GSV11_OnTick();
  }
void OnTimer()
  {
   ...
   else if(ActiveEngine==XVISION_EMA20_DIRECTIONAL) return;
   else                                             GSV11_OnTimer();
  }
//+------------------------------------------------------------------+
