//+------------------------------------------------------------------+
//| XVISION_EURUSD_Breakout_EA_v5_claude.mq4                          |
//|                                                                   |
...
//|   leaves you alone.                                               |
//|                                                                   |
//| MaxChasePips - why it exists                                      |
//|   Confirmation entries are late by construction: you wait for the |
//|   bar to close, and the bar may have run 30 pips first. Enter 30  |
//|   pips past a trigger with a 20 pip stop and the stop sits INSIDE |
//|   the range you were breaking out of - inside the noise the setup |
//|   exists to escape. MaxChasePips refuses that trade outright.     |
//|   Too late is too late; there is no rescuing it with sizing.      |
//|                                                                   |
//|   PENDING mode does not need it - the order fills at the level by |
//|   construction. It is still used there to spot a gap fill, where  |
//|   the broker fills far past the stop price and hands you the same |
//|   bad geometry without a signal ever being evaluated.             |
//|                                                                   |
...
input double MaxChasePips            = 8.0;    // 0 = off. Skip if price is already
                                               // this far past the trigger
...
string LOGFILE = "XVISION_EURUSD_v5_claude_Entries.csv";
...
void DetectOffset()
{
   ...
   {
      g_offsetSec = 0;
      Print("v5: TimeGMT() is unreliable in the tester - assuming server = GMT. ",
            "Set ServerGmtOffsetHours explicitly for a valid test.");
      return;
   }
   g_offsetSec = (int)(MathRound((double)(TimeCurrent()-TimeGMT())/3600.0)*3600);
   Print("v5: server is GMT", (g_offsetSec>=0?"+":""), g_offsetSec/3600,
...
bool ValidateConfig()
{
   ...
   if(StopLossPips > 0 && TakeProfitPips > 0 && TakeProfitPips < StopLossPips*0.5)
      Print("v5: NOTE take profit is under half the stop - fine if deliberate.");

   if(MaxChasePips > 0 && StopLossPips > 0 && MaxChasePips >= StopLossPips)
      Print("v5: WARNING MaxChasePips (",DoubleToString(MaxChasePips,1),
            ") is not below StopLossPips (",DoubleToString(StopLossPips,1),
            "). At that distance the stop lands inside the range and the filter ",
            "protects nothing.");
...
int OnInit()
{
   ...
   { Alert("v5: reasoned for EURUSD only, not ", Symbol()); return(INIT_FAILED); }
   ...
   if(iBars(NULL,TF) < 200)
      Print("v5: ONLY ", iBars(NULL,TF), " M15 BARS. The EA works on M15 whatever chart ",
   ...
   if(WriteEntryLog && !FileIsExist(LOGFILE))
   {
      ...
      if(fh != INVALID_HANDLE)
      {
         FileWrite(fh,"gmt_time","mode","dir","lots","entry","sl","tp",
                      "range_pips","adr_pips","range_to_adr","spread_pips","chase_pips","ticket");
         FileClose(fh);
      }
   }
   EventSetTimer(5);
...
//| A stop order guarantees a trigger price, never a fill price. On a
//| gap the broker fills wherever the book reopens, which can be far
//| past the level - the chase problem arriving without a signal.
void CheckGapFill()
{
   static int seen = -1;
   int t = OpenTicket();
   ...
   seen = t;
   if(!g_rangeOk || MaxChasePips <= 0) return;

   int    dir  = (OrderType()==OP_BUY) ? 1 : -1;
   double trig = (dir>0) ? BuyTrigger() : SellTrigger();
   double slip = (OrderOpenPrice()-trig)*dir/g_pip;
   if(slip > MaxChasePips)
      Say(StringConcatenate("v5: GAP FILL - filled ",DoubleToString(slip,1),
          " pips past the trigger, not at it. Your stop is that much closer to the ",
          "range than intended. Worth reviewing this trade."));
}
...
void WarnIfHedged()
{
   ...
   if(n > 1 && TimeCurrent()-lastWarn > 300)
   {
      ...
      Say(StringConcatenate("v5: WARNING ",n," positions open at once - both sides of the ",
...
//| Pips beyond whichever trigger is currently breached, else 0.
double ChasePips()
{
   if(!g_rangeOk) return(0);
   if(Ask > BuyTrigger())  return((Ask - BuyTrigger())/g_pip);
   if(Bid < SellTrigger()) return((SellTrigger() - Bid)/g_pip);
   return(0);
}
...
void PlacePendings()
{
   ...
   if(lotB <= 0 || lotS <= 0) { Print("v5: lot size resolved to zero"); return; }
   ...
   {
      Print("v5: triggers are inside the broker stop level (",
   ...
   {
      g_placed = true;
      Say(StringConcatenate("v5: bracket set  BUY STOP ",DoubleToString(bt,Digits),
          "   SELL STOP ",DoubleToString(st,Digits),
          "   range ",DoubleToString(RangePips(),1),"p (",DoubleToString(Ratio(),2)," ADR)"));
      if(tB>=0) LogEntry("PENDING", 1,lotB,bt,slB,tpB,tB,0);
      if(tS>=0) LogEntry("PENDING",-1,lotS,st,slS,tpS,tS,0);
   }
}
   ...
   for(int a=0; a<3; a++)
   {
      ...
      if(err == 147)
      {
         t = OrderSend(Symbol(),type,lots,price,SlippagePoints,sl,tp,
                       "XV5claude",MagicNumber,0,
                       (type==OP_BUYSTOP)?clrDodgerBlue:clrTomato);
         if(t >= 0) return(t);
         err = GetLastError();
      }
      Print("v5: pending ",type," failed, error ",err);
...
void HousekeepPendings()
{
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
   {
      ...
      if(!OrderDelete(OrderTicket(), clrGray))
         Print("v5: could not delete pending ",OrderTicket(),", error ",GetLastError());
      else if(filled)
         Print("v5: opposite side cancelled after fill");
   }
}
...
void CheckCloseBreak()
{
   ...
   // the whole point: refuse a chase rather than take it with a bad stop
   double chase = (dir>0) ? (entry-BuyTrigger())/g_pip : (SellTrigger()-entry)/g_pip;
   if(MaxChasePips > 0 && chase > MaxChasePips)
   {
      Print("v5: SKIPPED - price already ", DoubleToString(chase,1),
            " pips past the trigger (limit ", DoubleToString(MaxChasePips,1),
            "). Entering here would put the stop ",
            DoubleToString(MathMax(0, chase-StopLossPips),1),
            " pips inside the range.");
      return;
   }
   ...
   { Print("v5: stop or target inside broker minimum distance - skipped"); return; }
   ...
   for(int a=0; a<3; a++)
   {
      ...
      int t = OrderSend(Symbol(),(dir>0)?OP_BUY:OP_SELL,lots,entry,SlippagePoints,sl,tp,
                        "XV5claude",MagicNumber,0,(dir>0)?clrDodgerBlue:clrTomato);
      if(t >= 0)
      {
         Say(StringConcatenate("v5: ",(dir>0?"BUY ":"SELL "),DoubleToString(lots,2),
             " @ ",DoubleToString(entry,Digits),"  SL ",DoubleToString(sl,Digits),
             "  TP ",(tp>0?DoubleToString(tp,Digits):"none")));
         LogEntry("MARKET",dir,lots,entry,sl,tp,t,chase);
         return;
      }
      int err = GetLastError();
      if(err==130 || err==132 || err==145)      // ECN: open bare, then attach
      {
         ...
         t = OrderSend(Symbol(),(dir>0)?OP_BUY:OP_SELL,lots,(dir>0)?Ask:Bid,
                       SlippagePoints,0,0,"XV5claude",MagicNumber,0,clrNONE);
         if(t >= 0)
         {
            if(OrderSelect(t,SELECT_BY_TICKET) && !OrderModify(t,OrderOpenPrice(),sl,tp,0,clrNONE))
               Print("v5: WARNING ticket ",t," opened WITHOUT stops, error ",GetLastError());
            LogEntry("MARKET",dir,lots,OrderOpenPrice(),sl,tp,t,chase);
            return;
         }
         err = GetLastError();
      }
      Print("v5: entry failed, error ",err);
...
void ManageOpen()
{
   ...
   if(want != sl && want != 0)
   {
      if((dir>0 && want < px - minD) || (dir<0 && want > px + minD))
      {
         if(!OrderModify(t,entry,want,tp,0,clrNONE))
            Print("v5: stop update rejected, error ",GetLastError());
      }
   }
   ...
   if(UseTimeExit && TimeCurrent() >= ExitDeadline(OrderOpenTime()))
   {
      ...
      if(!OrderClose(t,OrderLots(),(dir>0)?Bid:Ask,SlippagePoints,clrGray))
         Print("v5: time exit failed, error ",GetLastError());
      else Say("v5: closed on time exit at "+Pad2(CloseAtHour)+":00 GMT");
   }
}
...
void LogEntry(string mode,int dir,double lots,double px,double sl,double tp,int ticket,double chase)
{
   if(!WriteEntryLog) return;
   int fh = FileOpen(LOGFILE, FILE_CSV|FILE_READ|FILE_WRITE|FILE_SHARE_READ, ',');
   if(fh == INVALID_HANDLE) { Print("v5: entry log not writable"); return; }
   ...
   FileWrite(fh, TimeToString(GmtNow(),TIME_DATE|TIME_SECONDS), mode, (dir>0?"BUY":"SELL"),
             ...
             DoubleToString(chase,1), IntegerToString(ticket));
   FileClose(fh);
}
...
string Action(color &c)
{
   ...
   if(EntryMode==ENTRY_ON_CLOSE && MaxChasePips>0 && ChasePips()>MaxChasePips)
   { c=PanelWarnColor; return("TOO LATE - "+DoubleToString(ChasePips(),1)+
                              " pips past trigger, not chasing"); }
...
void Panel()
{
   ...
   PTxt("t1", OriginX()+10,OriginY()+5,"XVISION EURUSD BREAKOUT v5",
   ...
   PRow(r++,"ORDER",StringConcatenate(
        ...
        "   TP ",(TakeProfitPips>0?DoubleToString(TakeProfitPips,0)+"p":"none"),
        "   chase ",(MaxChasePips>0?DoubleToString(MaxChasePips,0)+"p":"off")),PanelTextColor);
