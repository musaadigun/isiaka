...
#property description "XVISION Gold News Straddle V9.1 - hardened scheduling, whipsaw & entry-slippage guards"

// ============================================================================
// XVISION GOLD NEWS-STRADDLE EA VERSION 9.1
//
// V9.1 AUDIT FIXES
// - SCHEDULING CLOCK is now selectable (ScheduleClockSource). TimeGMT() is
//   unreliable in the Strategy Tester (there it tracks modeled server time)
//   and on machines with a wrong clock/timezone, which silently makes the EA
//   miss its window. You can now schedule off broker-server time, or server
//   time plus a manual GMT offset. Every scheduling decision flows through
//   SchedulingNow(); a one-line schedule summary and a tester warning are
//   printed at init so the resolved window is visible in the Experts log.
// - ENTRY-SLIPPAGE GUARD (MaximumFillSlippageMovement): if a leg fills more
//   than N dollars away from the price it was placed at, the whole event
//   stands down (closes/cancels) instead of riding a badly slipped news fill.
// - WHIPSAW POLICY (DoubleTriggerPolicy): when both legs fill, choose to keep
//   the first fill (old behaviour), keep the last, or close both and stand
//   down - the safest response to a two-sided news spike.
// - 64-BIT-SAFE EVENT KEY: the event token and global-state name no longer
//   truncate the news datetime through a 32-bit int (which overflowed for any
//   date on/after 2038-01-19, including the default placeholder).
// - PANEL REDRAW is throttled (it was rebuilt on every tick AND every second),
//   so the terminal stays responsive during the news tick-flood.
// - Pending geometry now respects FREEZELEVEL as well as STOPLEVEL and logs
//   the exact distances when a broker minimum blocks placement.
//
// V9 REDESIGN
...
//
// TIME STANDARD
// NewsDateTimeGMT is picked in F7. By default it is treated as GMT/UTC via
// TimeGMT(); ScheduleClockSource can switch the whole schedule to broker
// server time (recommended in the Strategy Tester) or server+manual offset.
//
// PRICE STANDARD
...
// Which clock drives scheduling. TimeGMT() is correct live but is modeled as
// server time inside the Strategy Tester, so backtests need an explicit choice.
enum ScheduleClock
{
   CLOCK_AUTO_GMT      = 0,   // Auto GMT via TimeGMT() (live only; wrong in tester)
   CLOCK_SERVER_TIME   = 1,   // Broker server time (enter News time in server time)
   CLOCK_MANUAL_OFFSET = 2    // Server time + manual offset below -> GMT
};

// What to do when BOTH straddle legs fill (a whipsaw).
enum WhipsawPolicy
{
   DT_KEEP_FIRST = 0,   // Keep the first leg that filled (original behaviour)
   DT_KEEP_LAST  = 1,   // Keep the most recent fill
   DT_CLOSE_BOTH = 2    // Close both and stand down (safest in a whipsaw)
};

// ============================================================================
//  PRIMARY TRADE SETUP  (this group heads the F7 "Inputs" tab)
// ============================================================================
input datetime      NewsDateTimeGMT             = D'2099.01.01 00:00'; // News time - pick with the date/time widget
...
input ScheduleClock ScheduleClockSource              = CLOCK_AUTO_GMT; // Clock used for all scheduling
input int           ManualServerGMTOffsetMinutes     = 0; // Server ahead of GMT by N min (clock=Manual)
...
input double MaximumFillSlippageMovement       = 0.0;   // Stand down if a leg fills >$ from its stop (0 disables)
...
input WhipsawPolicy DoubleTriggerPolicy            = DT_KEEP_FIRST; // Which leg to keep if both fill
...
double   g_intendedBuyStop=0.0; // price the buy-stop was placed at (entry-slippage guard)
double   g_intendedSellStop=0.0;// price the sell-stop was placed at
uint     g_lastUiTickMs=0;      // throttle for panel redraws
...
int OnInit()
{
   ...
   if(ForceResetEventState && GlobalVariableCheck(g_globalStateName))
   {
      ...
      Print("XVISION News Straddle V9.1: stored event state force-cleared.");
   }

   SynchroniseState();

   Print("XVISION V9.1 schedule | clock=",ClockName(),
         " | now=",TimeToString(SchedulingNow(),TIME_DATE|TIME_SECONDS),
         " | place=",TimeToString(g_placeGMT,TIME_DATE|TIME_MINUTES),
         " | news=",TimeToString(g_newsGMT,TIME_DATE|TIME_MINUTES),
         " | expiry=",TimeToString(g_expiryGMT,TIME_DATE|TIME_MINUTES));
   if(IsTesting() && ScheduleClockSource==CLOCK_AUTO_GMT)
      Print("XVISION V9.1 WARNING: TimeGMT() is modeled as server time in the "+
            "Strategy Tester. Set ScheduleClockSource=Broker server time and "+
            "enter the News time in server time for correct backtest scheduling.");
...
// All scheduling decisions run through one clock so the tester and a badly
// configured machine cannot silently shift the placement window.
datetime SchedulingNow()
{
   if(ScheduleClockSource==CLOCK_SERVER_TIME)
      return(TimeCurrent());
   if(ScheduleClockSource==CLOCK_MANUAL_OFFSET)
      return((datetime)(TimeCurrent()-ManualServerGMTOffsetMinutes*60));
   return(TimeGMT());
}

string ClockName()
{
   if(ScheduleClockSource==CLOCK_SERVER_TIME)   return("Broker server time");
   if(ScheduleClockSource==CLOCK_MANUAL_OFFSET) return("Server + manual offset -> GMT");
   return("Auto GMT (TimeGMT)");
}

string ClockShortLabel()
{
   if(ScheduleClockSource==CLOCK_SERVER_TIME) return("SRV");
   return("GMT");
}

// Panel/controls redraw, throttled so the news tick-flood cannot pin the UI.
// force=true guarantees an update (used from OnTimer once per second and init).
void RefreshUI(bool force)
{
   uint nowMs=GetTickCount();
   if(!force && (uint)(nowMs-g_lastUiTickMs)<400)
      return;
   g_lastUiTickMs=nowMs;
   UpdatePanel();
   UpsertControls();
   UpdateActionButtons();
}
...
bool ValidateInputs()
{
   ...
   if(MaximumFillSlippageMovement<0.0)
   { Print("Validation: MaximumFillSlippageMovement cannot be negative (use 0 to disable the guard)."); return(false); }

   if(ManualServerGMTOffsetMinutes<-900 || ManualServerGMTOffsetMinutes>900)
   { Print("Validation: ManualServerGMTOffsetMinutes must be between -900 and 900."); return(false); }
...
bool PlaceNewsStraddle()
{
   ...
   if(EnableBuyStop)
   {
      ...
      if(!PendingGeometryIsValid(OP_BUYSTOP,buyPrice,buySL,buyTP))
      {
         ...
         Print("BUY geometry rejected | entry=",DoubleToString(buyPrice,Digits),
               " ask=",DoubleToString(Ask,Digits)," SL=",DoubleToString(buySL,Digits),
               " brokerMinDist=",DoubleToString(BrokerMinDistance(),Digits));
         return(false);
      }

      RememberIntendedStop(OP_BUYSTOP,buyPrice);
   ...
   if(EnableSellStop)
   {
      ...
      if(!PendingGeometryIsValid(OP_SELLSTOP,sellPrice,sellSL,sellTP))
      {
         ...
         Print("SELL geometry rejected | entry=",DoubleToString(sellPrice,Digits),
               " bid=",DoubleToString(Bid,Digits)," SL=",DoubleToString(sellSL,Digits),
               " brokerMinDist=",DoubleToString(BrokerMinDistance(),Digits));
         return(false);
      }

      RememberIntendedStop(OP_SELLSTOP,sellPrice);
   ...
   Notify("XVISION News Straddle V9 armed | "+EventName+
          " | News "+ClockShortLabel()+" "+TimeToString(g_newsGMT,TIME_DATE|TIME_MINUTES)+
...
// The broker's minimum distance (in price) for stops/pendings. Honours both
// STOPLEVEL (placement distance) and FREEZELEVEL (modify/close distance).
double BrokerMinDistance()
{
   double d=MathMax(MarketInfo(Symbol(),MODE_STOPLEVEL),
                    MarketInfo(Symbol(),MODE_FREEZELEVEL))*Point;
   if(d<0.0) d=0.0;
   return(d);
}

// Record the price a pending leg was placed at, both in memory and in a
// GlobalVariable, so the entry-slippage guard survives a terminal restart.
void RememberIntendedStop(int orderType,double price)
{
   if(orderType==OP_BUYSTOP)
   {
      g_intendedBuyStop=price;
      GlobalVariableSet(g_globalStateName+"_BP",price);
   }
   else if(orderType==OP_SELLSTOP)
   {
      g_intendedSellStop=price;
      GlobalVariableSet(g_globalStateName+"_SP",price);
   }
}

double IntendedStopFor(int marketType)
{
   double v=(marketType==OP_BUY)?g_intendedBuyStop:g_intendedSellStop;
   if(v>0.0) return(v);
   string gv=g_globalStateName+((marketType==OP_BUY)?"_BP":"_SP");
...
{
   double stopDistance=BrokerMinDistance();
...
void ManageTriggeredTrades()
{
   ...
   int survivorTicket=SelectSurvivorTicket();

   if(CloseSecondTriggeredTrade && activeCount>1)
   {
      if(DoubleTriggerPolicy==DT_CLOSE_BOTH)
      {
         StandDownEvent("WHIPSAW - CLOSED BOTH LEGS");
         return;
      }
      if(survivorTicket>0)
   ...
   // Entry-slippage guard: a news stop that fills far from where it was placed
   // is a bad entry, so stand the whole event down rather than ride it. The
   // guard fails open (does nothing) if the intended price is unknown, e.g.
   // after a terminal restart with no persisted price.
   if(MaximumFillSlippageMovement>0.0)
   {
      double intended=IntendedStopFor(survivorType);
      if(intended>0.0)
      {
         double slip=MathAbs(OrderOpenPrice()-intended);
         if(slip>MaximumFillSlippageMovement)
         {
            StandDownEvent("ENTRY SLIPPED "+DoubleToString(slip,Digits)+" > "+
                           DoubleToString(MaximumFillSlippageMovement,Digits));
            return;
         }
      }
   }
...
// Which filled leg to keep when both triggered, per DoubleTriggerPolicy.
int SelectSurvivorTicket()
{
   if(DoubleTriggerPolicy==DT_KEEP_LAST)
      return(FindLatestActiveTicket());
   return(FindEarliestActiveTicket());
}
...
// Close every active leg and delete every resting pending for this event, then
// mark it COMPLETE so nothing re-arms. Used by the whipsaw "close both" policy
// and by the entry-slippage guard.
void StandDownEvent(string reason)
{
   int closed=0;
   ...
   for(int i=OrdersTotal()-1;i>=0;i--)
   {
      ...
      if(OrderType()==OP_BUYSTOP || OrderType()==OP_SELLSTOP)
         if(!OrderDelete(OrderTicket(),WarningColor)) ResetLastError();
   }

   SetState(STATE_COMPLETE);
   g_lastAction=reason;
   Notify("XVISION News Straddle V9 | "+reason+" | "+EventName+
...
bool MarketStopIsValid(int marketType,double stopLoss)
{
   ...
   double brokerDistance=BrokerMinDistance();
...
void SynchroniseState()
{
   ...
   if(GlobalVariableCheck(g_globalStateName))
   {
      ...
      if(stored>=STATE_PENDING && stored!=STATE_CANCELLED && SchedulingNow()<g_placeGMT)
      {
         GlobalVariableDel(g_globalStateName);
         g_state=STATE_WAITING;
      ...
      if(stored==STATE_PENDING && SchedulingNow()>=g_expiryGMT)
...
{
   // 64-bit-safe: the news datetime is rendered without truncation through a
   // 32-bit int (which overflowed for dates on/after 2038-01-19).
   string name="XVNS_"+IntegerToString(AccountNumber())+"_"+
               IntegerToString(MagicNumber)+"_"+
               IntegerToString((long)g_newsGMT);

   // Cap at 60 so the "_BP"/"_SP" intended-price suffixes still fit the 63-char
   // GlobalVariable name limit.
   if(StringLen(name)>60) name=StringSubstr(name,0,60);
   return(name);
}
...
void UpdatePanel()
{
   ...
   datetime nowSched=SchedulingNow();
   datetime brokerNow=TimeCurrent();
   int brokerOffset=(int)(brokerNow-nowSched);
   string  clk=ClockShortLabel();
   ...
   DrawLabel(PREFIX+"VAL_CD",y-1,PanelFontSize+2,CountdownText(nowSched),PanelAccent,true,true);
   y+=rowH+6;

   PanelSection(PREFIX+"S_EVT",y,"EVENT");  y+=secH;
   PanelRow(PREFIX+"EVT",  y,"Event",          EventName,                                        PanelText); y+=rowH;
   PanelRow(PREFIX+"NEWS", y,"News ("+clk+")",  TimeToString(g_newsGMT,TIME_DATE|TIME_MINUTES),   PanelText); y+=rowH;
   PanelRow(PREFIX+"PLC",  y,"Place ("+clk+")", TimeToString(g_placeGMT,TIME_DATE|TIME_MINUTES),  PanelText); y+=rowH;
   PanelRow(PREFIX+"EXP",  y,"Expire ("+clk+")",TimeToString(g_expiryGMT,TIME_DATE|TIME_MINUTES), PanelText); y+=rowH;
   ...
   PanelRow(PREFIX+"GMT",  y,(ScheduleClockSource==CLOCK_SERVER_TIME?"Server now":"GMT now"),
            TimeToString(nowSched,TIME_DATE|TIME_SECONDS), GMTHeaderText); y+=rowH;
...
void ApplySchedule(datetime newsGMT)
{
   ...
   // 64-bit-safe event token (see BuildGlobalStateName): the (int) cast used
   // here previously overflowed for any date on/after 2038-01-19.
   g_eventToken="XVN2_"+IntegerToString((long)g_newsGMT);
   g_globalStateName=BuildGlobalStateName();
   g_blockReason=""; g_notifiedReason="";
   g_missedNotified=false; g_heartbeatSent=false;
   g_intendedBuyStop=0.0; g_intendedSellStop=0.0;
   SynchroniseState();
}
